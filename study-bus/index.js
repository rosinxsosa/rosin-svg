!(function (e, t) {
  'object' == typeof exports && 'undefined' != typeof module
    ...
    : 'function' == typeof define && define.amd
      ? define(['exports'], t)
      : t(((e = 'undefined' != typeof globalThis ? globalThis : e || self).Studymux = {}));
})(this, function (e) {
  'use strict';
  ...
  async function c() {
    ...
    try {
      ...
      if (e instanceof AggregateError)
        throw (
          console.error('studymux: failed to get a studymux SharedWorker MessagePort as all clients returned an invalid MessagePort.'),
          new Error('All clients returned an invalid MessagePort.')
        );
      return (console.warn('studymux: failed to get a studymux SharedWorker MessagePort within 1s, retrying'), await c());
    }
  }
  function i(e) {
  ...
  function l(e, t) {
    const a = new r(e, 'studymux-wrker');
    return (
      t &&
        o.addEventListener('message', (t) => {
          if ('getPort' === t.data.type && t.data.port) {
            console.debug('studymux: recieved request for port from sw');
            const a = new r(e, 'studymux-wrker');
  ...
  class p {
    constructor(e) {
      ((this.channel = new BroadcastChannel('studymux')),
    ...
    createChannel(e, t) {
      if (self.clients)
        ...
        ((this.port = l(e, t)), console.debug('studymux: setting localStorage studymux-path to', e), (a['studymux-path'] = e));
      } else {
        if (!SharedWorker) throw new Error('Unable to get a channel to the SharedWorker.');
        {
          const e = a['studymux-path'];
          if ((console.debug('studymux: got localStorage studymux-path:', e), !e))
            throw new Error('Unable to get studymux workerPath from localStorage.');
          this.port = l(e, t);
        }
      }
    }
    async sendMessage(e, t) {
      ...
      try {
        ...
        return (
          console.warn('studymux: Failed to get a ping response from the worker within 1.5s. Assuming port is dead.'),
          this.createChannel(),
  ...
  (console.debug('studymux: running v2.1.7 (build c56d286)'),
    (e.StudyClient = m),
    (e.StudymuxConnector = class {
      constructor(e) {
        this.worker = new p(e);
      }
      async getTransport() {
      ...
      async bindTransfer(e, t, r) {
        await this.bindManualTransfer(
          `\n\t\t\tconst { default: StudyTransfer } = await import("${e}");\n\t\t\treturn [StudyTransfer, "${e}"];\n\t\t`,
          t,
          r
        );
      }
      async bindManualTransfer(e, t, r) {
        if ('studymuxremote' === e) throw new Error('Use setRemoteTransport.');
      ...
      async setRemoteTransport(e, t) {
        ...
        ((r.port1.onmessage = async (t) => {
          ...
          await this.worker.sendMessage({ type: 'set', client: { function: 'studymuxremote', args: [r.port2, t] } }, [r.port2]));
      }
      getInnerPort() {
