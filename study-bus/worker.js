!(function () {
  ...
  function c() {
    return new Error('there are no bare clients', {
      cause:
        'No StudyTransfer was set. Try creating a StudymuxConnector and calling bindTransfer() or bindManualTransfer() on it before using StudyClient.'
    });
  }
  function r(t, a) {
    const n = s;
    ...
    t.onmessage = async (t) => {
      ...
      if ('ping' === i.type) e.call(l, { type: 'pong' });
        ...
        try {
          ...
          if ('studymuxremote' === i.client.function) ((s = i.client.args[0]), (o = `studymuxremote (${i.client.args[1]})`));
          else {
  ...
  (new BroadcastChannel('studymux').postMessage({ type: 'refreshPort' }),
    (self.onconnect = (e) => {
      l(e.ports[0]);
    }),
